...
## Convenciones
- `/api/public/{tenant_slug}/...` para landing y checkout público.
- `/api/admin/...` para dashboard del negocio autenticado.
- `/api/platform/...` para superadmin.

## Endpoints mínimos
- GET catálogo público.
- POST carrito/checkout/pedido.
- POST crear preferencia Mercado Pago.
- POST webhook Mercado Pago.
- GET pedidos dashboard.
- PATCH estado de pedido.
- GET/POST productos, categorías, variantes.
- GET notificaciones.

...
## Backend
Django, Django REST Framework, SimpleJWT, django-cors-headers, mssql-django/pyodbc para SQL Server, Celery opcional para tareas async.

## Frontend
React, Vite, TypeScript, TailwindCSS, React Router, TanStack Query, Zustand o Context para estado liviano, React Hook Form + Zod para formularios.

## Base de datos
SQL Server. Usar UUIDs públicos cuando sea conveniente y claves internas seguras.

## Pagos
Mercado Pago Checkout Pro o Bricks según complejidad. MVP recomendado: Checkout Pro + webhook.

## Notificaciones
Campanita en dashboard con tabla `notifications`. WhatsApp mediante proveedor configurable: al inicio puede generarse enlace o integración abstracta, pero dejar preparada la interfaz para API real.
